...
import { CalendarDays, MapPin, Users, Plus, Search, Compass, Lock } from "lucide-react";
...
export const Route = createFileRoute("/community-events/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Discover and join FLINTA* community events." },
        ...
        <main className="flex-1 flex items-center justify-center px-5 py-20">
          <div className="max-w-md text-center bg-card border border-border/60 rounded-2xl p-8 shadow-soft">
            ...
            <p className="text-sm text-muted-foreground mb-6">
              Community events are visible to approved Pagu members. Please sign in to continue.
            </p>
                ...
                <li key={e.id}>
                  ...
                  >
                    <div className="flex items-center gap-2 mb-3">
                      <Badge variant="secondary" className="rounded-full">{e.category}</Badge>
                      <Badge variant="outline" className="rounded-full text-xs">{EVENT_TYPE_LABEL[e.type]}</Badge>
                    </div>
                    <h2 className="font-display text-xl leading-snug mb-2">{e.title}</h2>
                    <div className="text-xs text-muted-foreground space-y-1.5">
                      ...
                      <p className="inline-flex items-center gap-1.5">
                        ...
                        {attending}/{e.maxAttendees} attending · {remaining} spots left
                      </p>
                      <p className="text-muted-foreground/80">Hosted by {e.organizerName}</p>
                    </div>
                    <div className="mt-4 text-sm text-gold font-medium">View event →</div>
                  </Link>
                </li>
              );
            })}
